    ...
    return (
      <div className="flex flex-col gap-1.5">
        {label && (
          <label
            htmlFor={inputId}
            className="text-ui-sm font-medium text-warm-gray-700"
          >
            {label}
          </label>
        )}
        ...
        />
        {error && (
          <p className="text-ui-xs text-red-500 mt-0.5">{error}</p>
        )}
        {hint && !error && (
          <p className="text-ui-xs text-warm-gray-400 mt-0.5">{hint}</p>
        )}
      </div>
    );
  }
